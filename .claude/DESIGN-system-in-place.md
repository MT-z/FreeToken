...
`temperature=0`・`max_tokens=2048`（実際の生成長。中央値 177/186 トークン）で測り直すと:
...
新: prefill 119s + decode 367s
    → 減るのは prefill の 673 秒だけ。decode は 1 秒も減らない
```

しかも新は**出力を 1,653 トークン多く生成した上で**この時間である
（旧 27,275 / 新 28,928 トークン、入力はほぼ同数）。その意味で **2.30 倍は控えめ。**
...
文脈   0k– 40k  14本  2.11倍
文脈  40k– 80k   3本  1.99倍
文脈  80k–120k  29本  2.27倍
文脈 120k–      14本  1.57倍   ← 長いほど decode の比重が増えて頭打ち
```
...
**効きが最大になるのは、大きな文脈を読んで短い tool 呼び出しを1つ返すターン**——
エージェントで最も多い形。**薄いのは、長文脈で長く生成するターン。**
