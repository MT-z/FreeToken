...
主因だけを直すと、実 body 58 組で**改善 41 / 悪化 0 / 変化なし 17**（±100 トークン以内を「変化なし」とする。閾値を使わなければ 55 / 2 / 1 で、最大の悪化は −10 トークン）。
...
**58 組の由来**: 保存された body 60 本のうち、`/v1/messages` として読めない 1 本
（`req-00018`、`count_tokens` 呼び出し。根拠 JSON の `skipped` に記録）を除いた 59 本、
その連続する組が 58。10,000 バイト未満の本文は無かった。

```
58 組（±100 トークン以内を「変化なし」とする）:  改善 41  /  悪化 0  /  変化なし 17
  閾値を使わず素朴に数えると                  :  改善 55  /  悪化 2  /  同値     1
  最大の悪化                                  :  −10 トークン（1トークン以上悪化したのは2組）
```

**±100 という閾値はツールの定義**（`UNCHANGED_TOKENS`）であり、区切り文字のトークン化ゆらぎを
「変化なし」に寄せるためのもの。閾値を外すと 55 / 2 / 1 になるので、
**根拠 JSON から素朴に数え直すと数字が合わない点に注意**。1トークン以上悪化した2組は次のとおりで、
いずれもゆらぎの幅である。

| 組 | 現状 → 在置き | 差 |
|---|---|---:|
| req-00004 → 00005 | 27,704 → 27,694 | −10 |
| req-00060 → 00061 | 95,451 → 95,444 | −7 |

**「変化なし」17 組の内訳**（`shape_a` / `shape_b` から再計算できる）:

* **16 組** — A と B で**途中 system の本数が同じ**組。プロンプトの先頭がずれないので、
  現状コードでも 98.2〜99.9% 当たっている。在置きにしても当然変わらない
* **1 組（req-00056 → 00057）** — **別会話の境界**。
  messages 101→12、途中 system 34→3、tools 15→33 と構造ごと入れ替わっており、
  両方式とも 11.5% しか一致しない。**「既に当たっている」組ではない**

初版はここを「reminder が挟まっていない組」と書いたが**誤り**。
17 組すべての後続要求に途中 system が **3〜31 本**入っている（**0 本の組は無い**）。
（先行要求まで含めると 3〜34 本。34 は境界の組 req-00056 の先行側の値。）
効いているのは「挟まっていないこと」ではなく「**増えていないこと**」である。
...
* 全 58 組: `.claude/REVIEW-prefix-cache-evidence.json`（本文書と同じコミットに入っている。
  長さ・構造の本数・本文のハッシュのみ。**会話の中身は含まない**）
* 再導出: `freetoken-systest/tools/prefix-hoist-compare.py`（GPU 不要、トークナイザのみ）
* 一次出力: `freetoken-systest/results/20260907T014425-diag_prefix-cache-hoist-compare.json`
...
* **同一要求の再照合（`tok_match=21680` が 2,616 回連続）** — 正常動作。
  ...
  それが `try_add_one`（`prefill.py:261`）から呼ばれ、
  `prefill.py:328` の `for pending_req in self.pending_list:` により
  **周回ごとに待機中の全要求に対して**走る。
