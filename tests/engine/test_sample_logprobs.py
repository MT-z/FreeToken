...
def test_compute_logprobs_with_prepare_keeps_max_top_logprobs_clamped(monkeypatch) -> None:
    ...
    batch = SimpleNamespace(
        reqs=[
            SimpleNamespace(sampling_params=SimpleNamespace(logprobs=True, top_logprobs=17, is_greedy=True, needs_logits_processing=False)),
            SimpleNamespace(sampling_params=SimpleNamespace(logprobs=False, top_logprobs=0, is_greedy=True, needs_logits_processing=False)),
        ]
    )
